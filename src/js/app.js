...
import {createTweetPage, likedTweets, editTweet} from './create.js';
import {root, ref} from './index.js';
  ...
  tweets.forEach(function (item) {
    ref.tweetContainer.insertAdjacentHTML(
      ...
      <div class="formButtons">
        ...
        <button type="button" class="unlike ${item.liked ? 'like' : null}">&#10084;</button>
      </div>
    </li>`
    );
  });
...
function getFromLocalStorage() {
  if (localStorage.getItem('tweets')) {
    tweets.prop = JSON.parse(localStorage.getItem('tweets'));
    rendertweets(tweets);
  }
}
